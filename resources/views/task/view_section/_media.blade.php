...
@if( isset($parent) AND count($parent['taskMedias']) > 0 )
    <small>Archivos de la tarea principal</small>
    @foreach( $parent['taskMedias'] as $taskMedia )
        @if( $taskMedia->media !== null )
        <div class="block border rounded mb-2" >
            <div class="d-flex justify-content-between align-items-center">
                <div>
...
@if( isset($parent) AND count($parent['taskLinks']) > 0 )
    <small>Enlaces de la tarea principal</small>
    @foreach( $parent['taskLinks'] as $taskLink )
        <a href="{{ $taskLink->url }}" class="block task-link" target="_blank">
            <div class="card mt-2" style="background:transparent;box-shadow:none; border: 1px solid #e2e8f0;">
                <div class="card-body" style="padding:10px;">
                    <div class="d-flex justify-content-start align-items-center user-name">
